...
contract NFTDescriptorTest {
    ...
    function constructTokenURI(NFTDescriptor.URIParams calldata params) external pure returns (string memory) {
    ...
    function generateSVGImage(NFTDescriptor.URIParams memory params) external pure returns (string memory) {
        return NFTDescriptor.generateSVGImage(params);
    }

    function toColorHex(address token, uint256 offset) external pure returns (string memory) {
        return NFTDescriptor.toColorHex(uint256(uint160(token)), offset);
    }

    function isRare(uint256 tokenId, string memory name) external pure returns (bool) {
