...
library NFTSVG {
    ...
    function generateSVGDefs(SVGParams memory params) private pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                ...
                '</linearGradient>',
                generateSVGFilter('filter0_d', ['85.852', '212.189'], ['238.557', '53.1563'], '2'),
                generateSVGFilter('filter1_d', ['90.075', '103.557'], ['228.372', '171.911'], '6'),
    ...
    function generateSVGText(SVGParams memory params) private pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                '<g fill="black" font-family="monospace" font-style="bold" font-weight="bolder" style="text-shadow:6px 6px #558ABB; text-align:center;">',
    ...
    function generateSVGGradientEleOne(string memory id) private pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                '<linearGradient id="',
                id,
    ...
    function generateSVGGradientEleTwo(string memory id) private pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                '<linearGradient id="',
                id,
    ...
    function generateSVGBackGround(uint256 tokenId, string memory name) private pure returns (string memory svg) {
        if (isRare(tokenId, name)) {
            svg = string(abi.encodePacked('<rect id="r" x="0" y="0" width="419" height="512" ', 'fill="url(#g1)" />'));
        } else {
            svg = string(abi.encodePacked('<rect id="r" x="0" y="0" width="419" height="512" ', 'fill="black" />'));
        }
    }

    function isRare(uint256 tokenId, string memory name) internal pure returns (bool) {
